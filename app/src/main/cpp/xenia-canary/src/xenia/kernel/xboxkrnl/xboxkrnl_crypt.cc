...
// [UPSTREAM cebbdb6] HMAC-SHA1 state — two interleaved SHA1 states following
// the standard HMAC ipad/opad construction. The guest expects a 0xB0-byte
// contiguous region for XeCryptHmacShaInit/Update/Final.
...
// [UPSTREAM cebbdb6] Streaming HMAC-SHA1 — implements the standard ipad/opad
// construction over the existing XeCryptSha* primitives. Some titles call
// these streaming variants instead of the one-shot XeCryptSha to hash
// chunked data (e.g. network packet authentication); without them the
// calls resolve to the universal "unimplemented" stub and the title fails
// its handshake (Xbox Live sign-in, profile import, etc.).
void XeCryptHmacShaInit_entry(pointer_t<XECRYPT_HMACSHA_STATE> sha_state_ptr,
  ...
  // ipad = key XOR 0x36, opad = key XOR 0x5C — standard HMAC construction.
  for (uint8_t i = 0; i < key_size_; i += 1) {
    key_1[i] ^= 0x5C5C5C5Cu;
    key_2[i] ^= 0x36363636u;
  }

  // sha_state[0] gets the opad pre-load, sha_state[1] gets the ipad pre-load.
  // (Final swaps them around so the outer hash absorbs the inner result.)
...
void XeCryptHmacShaFinal_entry(pointer_t<XECRYPT_HMACSHA_STATE> sha_state_ptr,
                               pointer_t<uint8_t> out, dword_t out_size) {
  // Finalize the inner (ipad) hash, then feed it as a single block into the
  // outer (opad) hash and finalize that — yielding HMAC-SHA1(key, message).
