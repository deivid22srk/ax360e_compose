...
struct XenosZPDReport {
  ...
  // ax360e backport: after commit 73945c06, saturation is renamed from
  // occlusion_query_sample_count_saturation to occlusion_query_saturation.
  static uint32_t SaturateSampleCount(uint32_t sample_count) {
    double saturation = std::clamp(
        static_cast<double>(cvars::occlusion_query_saturation),
        0.0, 1.0);
