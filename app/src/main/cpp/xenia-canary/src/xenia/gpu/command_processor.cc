...
// ax360e backport: ZPD occlusion query lifecycle implementation
// (upstream fbd620c2 + 73945c06 simplify).
...
bool CommandProcessor::EndZPDReport(uint32_t report_address,
  ...
  if (mode == ZPDMode::kFast || mode == ZPDMode::kFastAlt) {
    ...
    if (!resolved_immediately) {
      speculative = 1;
      // ax360e backport (73945c06): kFastAlt preserves cached zeroes.
      if (has_cached_delta &&
          (cached_delta != 0 || mode == ZPDMode::kFastAlt)) {
    ...
    if (!logical_zpd_reports_.count(report_handle)) {
      // OnZPDQueryResolved already committed and erased the report.
    } else if (zpd_pending_retire_handle_ != report_handle) {
      zpd_pending_retire_handle_ = report_handle;
      zpd_pending_retire_stalls_ = 0;
      zpd_pending_retire_start_ms_ = xe::Clock::QueryHostUptimeMillis();
    }
  }
...
void CommandProcessor::PumpPendingRetire() {
  ...
  bool deadline_exceeded =
      (xe::Clock::QueryHostUptimeMillis() - zpd_pending_retire_start_ms_ >=
