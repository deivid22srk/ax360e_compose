...
// [UPSTREAM 9781a75] Avoid stale texture state after invalidation
// Returns whether every page in the range is currently valid in the host GPU
// memory copy. Used by TextureCache::Texture::MakeUpToDateAndWatch to avoid
// marking a texture "up to date" right after its backing memory was
// invalidated by a guest invalidation — previously the texture would be
// marked up to date and a watch installed, but the data was already gone,
// producing stale visual state on the next draw. Hold the global critical
// region if relying on this for state transitions such as watch installation.
