...
// ax360e backport: new ZPD occlusion query system (upstream fbd620c2 +
// 73945c06 simplify). Replaces the old oscillating fake sample count with
// real GPU occlusion queries (fast/strict modes) or improved fake mode.
XE_NOINLINE
bool COMMAND_PROCESSOR::ExecutePacketType3_EVENT_WRITE_ZPD(
  ...
  if (mode != ZPDMode::kFake && !zpd_force_fake_fallback_) {
    ...
    if (!logical_active && is_end_record) {
      ...
      // ax360e backport (73945c06): kFastAlt also uses cached delta here.
    ...
    // to the fake path so the guest at least gets a result written.
  }
