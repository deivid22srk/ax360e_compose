...
void D3D12CommandProcessor::WriteRegisterForceinline(uint32_t index,
...
#elif XE_ARCH_ARM64
    int16_t index16 = static_cast<int16_t>(index);

    int16x8_t to_rangecheck = vdupq_n_s16(index16);

    const alignas(16) int16_t lower_bounds[8] = {
            XE_GPU_REG_SHADER_CONSTANT_FETCH_00_0 - 1,
            XE_GPU_REG_SHADER_CONSTANT_000_X - 1,
            ...
            0, 0};
    const alignas(16) int16_t upper_bounds[8] = {
            XE_GPU_REG_SHADER_CONSTANT_FETCH_31_5 + 1,
            XE_GPU_REG_SHADER_CONSTANT_511_W + 1,
            XE_GPU_REG_SHADER_CONSTANT_LOOP_31 + 1,
            XE_GPU_REG_SCRATCH_REG7 + 1,
            XE_GPU_REG_COHER_STATUS_HOST + 1,
            XE_GPU_REG_DC_LUT_30_COLOR + 1, 0, 0};
    const alignas(16) int16_t shift_arr[8] = {-7, -5, -3, -1, 1, 3, 5, 7};

    int16x8_t lower = vld1q_s16(lower_bounds);
    int16x8_t upper = vld1q_s16(upper_bounds);
    int16x8_t shift_mask = vdupq_n_u16(0x180);
    int16x8_t shift = vld1q_s16(shift_arr);

    uint16x8_t is_above_lower = vreinterpretq_u16_s16(vcgtq_s16(to_rangecheck, lower));
    uint16x8_t is_below_upper = vreinterpretq_u16_s16(vcltq_s16(to_rangecheck, upper));
    uint16x8_t is_within_range = vandq_u16(is_above_lower, is_below_upper);

    register_file_->values[index] = value;

    uint16x8_t is_within_range_shifted = vshlq_u16(vandq_u16(is_within_range, shift_mask), shift);
    uint32_t movmask = vaddvq_u16(is_within_range_shifted);
#else
    register_file_->values[index] = value;

  uint32_t movmask = 0;

  if (index > (XE_GPU_REG_SHADER_CONSTANT_FETCH_00_0 - 1) &&
      index < (XE_GPU_REG_SHADER_CONSTANT_FETCH_31_5 + 1)) {
    movmask |= (1 << 0) | (1 << 1);
  }

  if (index > (XE_GPU_REG_SHADER_CONSTANT_000_X - 1) &&
      index < (XE_GPU_REG_SHADER_CONSTANT_511_W + 1)) {
    movmask |= (1 << 2) | (1 << 3);
  }

  if (index > (XE_GPU_REG_SHADER_CONSTANT_BOOL_000_031 - 1) &&
      index < (XE_GPU_REG_SHADER_CONSTANT_LOOP_31 + 1)) {
    movmask |= (1 << 4) | (1 << 5);
  }

  if (index > (XE_GPU_REG_SCRATCH_REG0 - 1) &&
      index < (XE_GPU_REG_SCRATCH_REG7 + 1)) {
    movmask |= (1 << 6) | (1 << 7);
  }
  ...
  if (index > (XE_GPU_REG_DC_LUT_RW_INDEX - 1) &&
      index < (XE_GPU_REG_DC_LUT_30_COLOR + 1)) {
    movmask |= (1 << 10) | (1 << 11);
  }
