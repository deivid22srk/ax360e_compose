...
RegisterFile::RegisterFile() {
  ...
  // [UPSTREAM e20f269] Initialize GPU registers to hardware reset defaults.
  ...
  // otherwise observe 0 under emulation but the real power-on default on
  // hardware. These values were read off a retail console (read-only) and
  // cross-checked against the AMD R6xx/R7xx 3D register reference and the
  // Mesa r600g driver, which programs the same registers to the same values
  // at context init. Fixes a class of "game reads register before writing
  // it" bugs that produced wrong viewport sizes, missing AA masks, broken
  // point/line rendering, etc.
  ...
  // Tessellation levels also reset to 1.0f on hardware, but the backends
  // apply the effective factor as (register + 1.0f), so initializing them
  // here would change the effective reset factor; left out for separate
  // review.
